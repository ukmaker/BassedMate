...
Sequencer() {
  ...
  // asign the default instrument from each family
  

  _instruments[0] = drumsCollection.getDefaultInstrument();
  _instruments[1] = cymbalsCollection.getDefaultInstrument();
  _instruments[2] = woodsCollection.getDefaultInstrument();
  _instruments[3] = specialsCollection.getDefaultInstrument();
  
  for(int channel = 0; channel < 4; channel++) {
    _lastNotes[channel] = getDefaultNote(channel);
    
    for(int x = 0; x < 8; x++) {
      _notes[x][channel] =  getDefaultNote(channel);
      _active[x][channel] = false;
    }
  }
}
